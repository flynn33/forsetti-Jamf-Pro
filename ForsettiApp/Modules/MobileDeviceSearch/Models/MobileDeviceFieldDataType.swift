...
/// `JamfRSQLComposer` (which RSQL operators are valid for the field). New cases
/// must be paired with a `supportedOperators` mapping below; otherwise the picker
/// will offer no operators for the field and it will be silently un-queryable.
enum MobileDeviceFieldDataType: String, Codable, Sendable, CaseIterable {
    ...
    case integer
    /// Floating-point values. Currently unused by the catalog but reserved for
    /// future fields like geolocation accuracy or decimal scores.
    case double
