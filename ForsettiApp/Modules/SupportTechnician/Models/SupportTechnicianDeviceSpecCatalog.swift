...
///
/// `SupportTechnicianDeviceSpecCatalog` returns one of these for every identifier it
...
nonisolated struct SupportTechnicianDeviceSpec: Sendable {
    ...
    /// Text fallback for GPU variants that cannot be represented by one
    /// exact integer from Jamf's reported fields alone.
    let gpuCoreDescription: String?

    /// Rated CPU frequency text for models where Jamf reports 0 MHz or omits
    /// speed entirely. Apple Silicon has mixed core clusters, so this is a
    /// descriptive string rather than a single MHz integer.
    let cpuFrequencyDescription: String?

    /// Rated memory technology/bandwidth text. Jamf reports installed RAM,
    /// but Apple Silicon memory does not map cleanly to DIMM MHz.
    ...
    init(
        ...
        gpuCoreCount: Int?,
        neuralCoreCount: Int?,
        gpuCoreDescription: String? = nil,
        cpuFrequencyDescription: String? = nil,
        memoryDescription: String? = nil,
        ramGB: @escaping @Sendable (_ capacityMb: Int?) -> Int?
    ) {
        ...
        self.gpuCoreDescription = gpuCoreDescription
        self.cpuFrequencyDescription = cpuFrequencyDescription
...
/// branches.
enum SupportTechnicianDeviceSpecCatalog {

    // Support Technician owns this lookup table so the module stays independent
    // from Mobile Device Search. Its mobile rows mirror the Mobile-device
    // derivation process: model identifier + storage capacity resolves
    // marketing name, chip, core counts, and RAM tier.
    ...
    /// `SupportTechnicianDeviceSpec.ramGB` so the catalog dictionary itself can be
    ...
    nonisolated private static let ramFromJamf: @Sendable (Int?) -> Int? = { _ in nil }

    // MARK: - Lookup table

    nonisolated static let entries: [String: SupportTechnicianDeviceSpec] = [

        // MARK: Mac

        // MacBook Pro 14-inch / 16-inch (2021). Jamf reports these machines
        // with `processorType` but often leaves CPU cores at 0 and has no
        // GPU-core field. The identifier + chip name fills the support view
        // with the same static-spec approach used for mobile devices.
        "MacBookPro18,1": .init(
            marketingName: "MacBook Pro 16-inch (2021)", chipName: "Apple M1 Pro",
            cpuCoreCount: 10, gpuCoreCount: nil, neuralCoreCount: 16,
            gpuCoreDescription: "16 GPU cores",
            cpuFrequencyDescription: "Performance cores up to 3.2 GHz",
            memoryDescription: "Unified memory · up to 200 GB/s bandwidth",
            ramGB: ramFromJamf
        ),
        "MacBookPro18,2": .init(
            marketingName: "MacBook Pro 16-inch (2021)", chipName: "Apple M1 Max",
            cpuCoreCount: 10, gpuCoreCount: nil, neuralCoreCount: 16,
            gpuCoreDescription: "24 or 32 GPU cores",
            cpuFrequencyDescription: "Performance cores up to 3.2 GHz",
            memoryDescription: "Unified memory · up to 400 GB/s bandwidth",
            ramGB: ramFromJamf
        ),
        "MacBookPro18,3": .init(
            marketingName: "MacBook Pro 14-inch (2021)", chipName: "Apple M1 Pro",
            cpuCoreCount: 8, gpuCoreCount: nil, neuralCoreCount: 16,
            gpuCoreDescription: "14 GPU cores",
            cpuFrequencyDescription: "Performance cores up to 3.2 GHz",
            memoryDescription: "Unified memory · up to 200 GB/s bandwidth",
            ramGB: ramFromJamf
        ),
        "MacBookPro18,4": .init(
            marketingName: "MacBook Pro 14-inch (2021)", chipName: "Apple M1 Max",
            cpuCoreCount: 10, gpuCoreCount: nil, neuralCoreCount: 16,
            gpuCoreDescription: "24 or 32 GPU cores",
            cpuFrequencyDescription: "Performance cores up to 3.2 GHz",
            memoryDescription: "Unified memory · up to 400 GB/s bandwidth",
    ...
    nonisolated static func info(for modelIdentifier: String) -> SupportTechnicianDeviceSpec? {
