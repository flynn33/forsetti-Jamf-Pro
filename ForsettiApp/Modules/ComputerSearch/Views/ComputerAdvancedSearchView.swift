...
/// Sheet that hosts the computer Advanced Search builder.
///
/// Layout: a List of group sections — each group has a combinator picker and
/// rows for its criteria — followed by an outer-combinator picker if there is
/// more than one group, and a sticky bottom bar with validation hints plus the
/// Search and Save Smart Filter actions. Computer-side analogue of
/// `AdvancedSearchView` (mobile).
struct ComputerAdvancedSearchView: View {
    ...
    /// Called with the named smart filter when the user hits Save. The parent
    /// persists it via `ComputerSmartFilterStore`.
    ...
    private var bottomBar: some View {
        VStack(alignment: .leading, spacing: 8) {
            // RSQL preview is kept out of the user-visible UI — the composed
            // query is logged to Diagnostics for support purposes; the bottom
            // bar stays focused on validation hints and the Search action.
