...
// navigation shell used by both macOS and iOS builds.
//
...
final class DeploymentTrackerModule: JamfModule {
    let id: String
    let title: String
    let subtitle: String
    let iconSystemName: String

    // The default module metadata is used by the framework registry, bundled
    // package manifests, and the module switcher. Changing these values affects
    ...
    // The framework injects shared infrastructure here. Deployment Tracker
    // deliberately consumes the common Jamf API gateway, credentials store, and
    // diagnostics reporter instead of creating duplicate clients or credentials.
...
final class DeploymentTrackerViewModel: ObservableObject {
    // Shared framework services. These are passed in by ModuleContext so this
    // module can run inside the app without constructing parallel service stacks.
        ...
        do {
            ...
            return StoreResolution(
                ...
                startupIssue: DeploymentUserFacingIntegrationError(
                    ...
                    externalDataChanged: false,
                    safeToRetry: true,
                    recommendedAction: "Check the local database folder permissions and available disk space, then restart Forsetti. Export diagnostics if this repeats.",
