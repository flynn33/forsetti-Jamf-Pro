...
// MARK: - Embedded Apple device model catalog
//
// This catalog maps known Apple model identifiers to hardware details used by
// Deployment Tracker enrichment. It is intentionally local and deterministic so
// hardware derivation continues to work without network access.
/// Resolved hardware information for an Apple device, derived from its
/// `modelIdentifier` (e.g. `iPad14,5`).
///
/// `DeploymentAppleDeviceModelCatalog` returns one of these for every identifier it
...
struct DeploymentAppleDeviceModelInfo: Sendable {
...
/// branches.
enum DeploymentAppleDeviceModelCatalog {
    ...
    /// `DeploymentAppleDeviceModelInfo.ramGB` so the catalog dictionary itself can be
    ...
    nonisolated static let entries: [String: DeploymentAppleDeviceModelInfo] = [
    ...
    nonisolated static func info(for modelIdentifier: String) -> DeploymentAppleDeviceModelInfo? {
