...
/// A comprehensive error type covering all failure modes in the Forsetti framework.
///
/// Each case maps to a specific category of failure -- from authentication and
/// network issues to keychain operations and module package validation. The
...
enum JamfFrameworkError: LocalizedError {
    ...
    /// A module package failed validation checks.
    /// - Parameter message: Details about the validation failure.
    case invalidModulePackage(message: String)
    /// A module package with the same identifier is already registered.
    /// - Parameter packageID: The conflicting package identifier.
    case duplicateModulePackage(packageID: String)
    /// The module package declares a type that the framework does not support.
    /// - Parameter type: The unsupported type string.
