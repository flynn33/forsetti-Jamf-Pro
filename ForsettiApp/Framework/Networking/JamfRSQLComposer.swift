...
enum JamfRSQLComposer {
    ...
    private static func render(
        ...
        case .equals:
            ...
            return JamfRSQLFilter.equality(field: field.key, value: value, wildcard: .none, negated: false)
        case .notEquals:
            guard let value = stringValue() else { return nil }
            return JamfRSQLFilter.equality(field: field.key, value: value, wildcard: .none, negated: true)
        case .contains:
            guard let value = stringValue() else { return nil }
            return JamfRSQLFilter.equality(field: field.key, value: value, wildcard: .both, negated: false)
        case .notContains:
            guard let value = stringValue() else { return nil }
            return JamfRSQLFilter.equality(field: field.key, value: value, wildcard: .both, negated: true)
        case .startsWith:
            guard let value = stringValue() else { return nil }
            return JamfRSQLFilter.equality(field: field.key, value: value, wildcard: .trailing, negated: false)
        case .endsWith:
            guard let value = stringValue() else { return nil }
            return JamfRSQLFilter.equality(field: field.key, value: value, wildcard: .leading, negated: false)
        ...
        case .lessThan, .lessThanOrEqual, .greaterThan, .greaterThanOrEqual:
            ...
            return "\(field.key)\(criterion.op.rsqlSymbol)'\(literal)'"
        ...
        case .between:
            ...
            return "(\(field.key)=ge='\(lower)';\(field.key)=le='\(upper)')"
        ...
        case .includedIn:
            ...
            let parts = values.map {
                JamfRSQLFilter.equality(field: field.key, value: $0, wildcard: .none, negated: false)
            }
        ...
        case .excludedFrom:
            ...
            guard values.isEmpty == false else { return nil }
            let parts = values.map {
                JamfRSQLFilter.equality(field: field.key, value: $0, wildcard: .none, negated: true)
            }
            return "(" + parts.joined(separator: ";") + ")"
        }
    }
