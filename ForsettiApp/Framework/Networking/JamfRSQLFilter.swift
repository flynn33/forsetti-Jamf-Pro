...
/// on strict tenants.
///
/// `nonisolated` because the module's default actor isolation is
/// `MainActor` (see `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor`), which
/// would otherwise make these pure static functions callable only from
/// the main actor. `actor SupportTechnicianAPIService` needs to call
/// them from its own isolation domain, so they must be nonisolated.
enum JamfRSQLFilter {
