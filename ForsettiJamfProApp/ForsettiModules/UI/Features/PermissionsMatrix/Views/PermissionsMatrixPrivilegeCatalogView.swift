...
/// Privilege catalog: browse Jamf Pro privilege names and see which Forsetti Jamf Pro
...
struct PermissionsMatrixPrivilegeCatalogView: View {
    ...
    private func relatedActions(_ privilege: String) -> some View {
        ...
        PHSectionCard(title: "Forsetti Jamf Pro actions (\(actions.count))", systemImage: "command") {
            if actions.isEmpty {
                Text("No Forsetti Jamf Pro actions reference this privilege.")
                    .font(.callout).foregroundStyle(.secondary)
            } else {
                VStack(spacing: 4) {
