...
/// Drives the Advanced Search sheet.
///
/// Owns the in-progress `AdvancedQuery`, exposes a live RSQL preview, and
/// validates as criteria change. Also handles saving the query as a named
/// `SmartFilter` once the user decides to keep it.
///
/// Search execution itself stays on `MobileDeviceSearchViewModel` so the
/// existing pagination / section fallback / prestage resolution code is
/// reused unchanged. This view model just hands the composed query and
/// chosen field columns to the parent on Search.
@MainActor
final class AdvancedSearchViewModel: ObservableObject {
    ...
    let fieldLookup: [String: MobileDeviceField]
    ...
    /// setting, `MobileDeviceField.catalog` and `keyLookup` are MainActor-
    /// isolated; using them as default-argument expressions triggers
    /// "main actor-isolated static property can not be referenced from a
    /// nonisolated context" because the compiler evaluates the default at
    /// the declaration site, not the call site. Every caller already passes
    /// the merged catalog from the parent view model anyway.
    init(
        initialQuery: AdvancedQuery,
        initialFieldKeys: Set<String>,
        availableFields: [MobileDeviceField],
    ...
    func compose() -> JamfRSQLComposer.ComposeResult {
        JamfRSQLComposer.compose(query, fieldLookup: fieldLookup)
    }

    private func regeneratePreview(for query: AdvancedQuery) {
        let result = JamfRSQLComposer.compose(query, fieldLookup: fieldLookup)
