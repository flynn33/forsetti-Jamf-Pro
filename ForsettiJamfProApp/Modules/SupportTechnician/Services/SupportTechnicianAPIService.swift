...
actor SupportTechnicianAPIService {
    ...
    func createApplicationActionScript(
        ...
        let payload: [String: Any] = [
            "name": name,
            "info": "Auto-added by Forsetti Jamf Pro to \(actionVerb.lowercased()) \(appName) on \(deviceDisplayName) (serial \(serialNumber)). Safe to delete after the paired one-shot policy has executed.",
    ...
    private func generateApplicationActionScript(
        ...
        return """
        #!/bin/zsh
        # Forsetti Jamf Pro one-shot \(verb) wrapper.
    ...
    private static var applicationActionCleanupQueueURL: URL {
        ...
        let jamfDirectory = baseDirectory.appendingPathComponent("JamfDashboard", isDirectory: true)
    ...
    nonisolated static func dumpPayloadForDiagnostics(
        ...
        let folder = documents.appendingPathComponent("JamfDashboardDiagnostics", isDirectory: true)
