...
/// Privilege catalog: browse Jamf Pro privilege names and see which Jamf Dashboard
...
struct PermissionsMatrixPrivilegeCatalogView: View {
    ...
    private func relatedActions(_ privilege: String) -> some View {
        ...
        PHSectionCard(title: "Jamf Dashboard actions (\(actions.count))", systemImage: "command") {
            if actions.isEmpty {
                Text("No Jamf Dashboard actions reference this privilege.")
                    .font(.callout).foregroundStyle(.secondary)
            } else {
                VStack(spacing: 4) {
