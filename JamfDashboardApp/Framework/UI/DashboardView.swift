...
private struct ModuleCard: View {
    ...
    private var status: ForsettiSemanticStatus {
        moduleID.contains("deployment-tracker") ? .workflow : .ready
    }
        ...
        VStack(alignment: .leading, spacing: DashboardTheme.Spacing.md) {
            HStack(alignment: .top, spacing: DashboardTheme.Spacing.md) {
                Image(systemName: iconSystemName)
                    ...
                    .overlay(RoundedRectangle(cornerRadius: DashboardTheme.Radius.medium, style: .continuous).stroke(status.color.opacity(0.34), lineWidth: 1))

                Spacer(minLength: 0)
                ForsettiStatusPill(status: status, text: status == .workflow ? "Workflow" : "Ready")
            }
