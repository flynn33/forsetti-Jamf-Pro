...
actor SupportTechnicianAPIService {
    ...
    func createApplicationActionScript(
        ...
        let payload: [String: Any] = [
            "name": name,
            "info": "Auto-added by Jamf Dashboard to \(actionVerb.lowercased()) \(appName) on \(deviceDisplayName) (serial \(serialNumber)). Safe to delete after the paired one-shot policy has executed.",
    ...
    private func generateApplicationActionScript(
        ...
        return """
        #!/bin/zsh
        # Jamf Dashboard one-shot \(verb) wrapper.
