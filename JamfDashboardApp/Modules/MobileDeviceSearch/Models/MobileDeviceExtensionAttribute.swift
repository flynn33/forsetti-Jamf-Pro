...
/// runtime via `GET /api/v2/mobile-device-extension-attributes` and converted
/// into transient `MobileDeviceField` entries so the user can build Advanced
/// Search criteria against them. Because Jamf's RSQL syntax for filtering by
/// extension attributes varies by version (and the public docs don't pin it
/// down), EA criteria are routed to the in-memory client-side matcher rather
/// than emitted as server-side RSQL — see `JamfRSQLComposer` and the
/// `isServerFilterable` flag.
struct MobileDeviceExtensionAttribute: Identifiable, Decodable, Hashable, Sendable {

    /// Mirrors Jamf's documented EA `dataType` enum. Drives the
    /// `MobileDeviceFieldDataType` mapping below so the Advanced Search picker
    /// shows the correct input control (text / number / date).
    ...
    /// Catalog-field key used everywhere downstream. The `eaPrefix` is what
    /// `MobileDeviceSearchViewModel.parseRecord` uses to recognize EA values
    /// when flattening the response, and what the client-side matcher uses
    /// when looking up the criterion's value on a record.
    ...
    /// `.string` for unknown variants so the picker still offers a sane
    /// operator allowlist.
    var swiftDataType: MobileDeviceFieldDataType {
        switch dataType {
    ...
    /// Builds a `MobileDeviceField` matching this EA's metadata. The result is
    /// merged into `MobileDeviceSearchViewModel.allFields` so it shows up in
    /// the Advanced Search picker.
    ///
    /// `isFilterable` is `true` (so the picker shows it) but
    /// `isServerFilterable` is `false` so the composer routes EA criteria to
    /// the client-side matcher. The user pays a slight perf cost (we fetch
    /// devices that match the rest of the query first, then filter EAs in
    /// memory) but avoids version-specific RSQL syntax breakage.
    func makeField() -> MobileDeviceField {
        let descriptionText = (description?.isEmpty == false ? description! : "Custom extension attribute")
        return MobileDeviceField(
            key: fieldKey,
            displayName: name,
