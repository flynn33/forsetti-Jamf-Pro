...
struct DashboardMetalBackgroundView: View {
    var body: some View {
#if canImport(MetalKit)
        if MTLCreateSystemDefaultDevice() != nil {
            DashboardMetalBackgroundRepresentable()
...
/// Fields must match the MSL `DashboardMetalUniforms` struct exactly.
private struct DashboardMetalUniforms {
...
private final class DashboardMetalBackgroundRenderer: NSObject, MTKViewDelegate {
    private static let logger = Logger(subsystem: "com.jamfdashboard.app", category: "MetalRenderer")
    ...
    init?(device: MTLDevice) {
        ...
        guard let vertexFunction = library.makeFunction(name: "dashboardMetalBackgroundVertex") else {
            Self.logger.error("Metal background renderer failed to locate vertex function.")
            return nil
        }

        guard let fragmentFunction = library.makeFunction(name: "dashboardMetalBackgroundFragment") else {
    ...
    struct DashboardMetalUniforms {
        float2 resolution;
        float time;
        float padding;
    };

    struct DashboardMetalVertexOutput {
        float4 position [[position]];
        float2 uv;
    };

    vertex DashboardMetalVertexOutput dashboardMetalBackgroundVertex(uint vertexID [[vertex_id]]) {
        ...
        DashboardMetalVertexOutput output;
        output.position = float4(positions[vertexID], 0.0, 1.0);
        output.uv = positions[vertexID] * 0.5 + 0.5;
        return output;
    }

    fragment float4 dashboardMetalBackgroundFragment(
        DashboardMetalVertexOutput input [[stage_in]],
        constant DashboardMetalUniforms& uniforms [[buffer(0)]]
    ) {
        float2 uv = input.uv;
    ...
    func draw(in view: MTKView) {
        ...
        var uniforms = DashboardMetalUniforms(
        ...
        encoder.setFragmentBytes(&uniforms, length: MemoryLayout<DashboardMetalUniforms>.stride, index: 0)
...
private final class DashboardMetalBackgroundCoordinator {
    /// The Metal renderer driving the animation (nil if GPU init failed).
    let renderer: DashboardMetalBackgroundRenderer?
    ...
    init() {
        guard let device = MTLCreateSystemDefaultDevice(),
              let renderer = DashboardMetalBackgroundRenderer(device: device) else {
            self.renderer = nil
...
private struct DashboardMetalBackgroundRepresentable: UIViewRepresentable {
    /// Creates the coordinator that owns the Metal view and renderer.
    func makeCoordinator() -> DashboardMetalBackgroundCoordinator {
        DashboardMetalBackgroundCoordinator()
    }
...
private struct DashboardMetalBackgroundRepresentable: NSViewRepresentable {
    /// Creates the coordinator that owns the Metal view and renderer.
    func makeCoordinator() -> DashboardMetalBackgroundCoordinator {
        DashboardMetalBackgroundCoordinator()
    }
